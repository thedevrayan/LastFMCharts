...
router.post('/playlists', async (req, res) => {
    ...
    const playlist = new Playlist_1.PlaylistModel({
        name,
        description,
...
router.get('/playlists/:id/tracks', async (req, res) => {
    try {
        const { id } = req.params;
        const playlist = await Playlist_1.PlaylistModel.findById(id);
        if (!playlist) {
...
router.put('/playlists/:id', async (req, res) => {
    try {
        ...
        const updated = await Playlist_1.PlaylistModel.findByIdAndUpdate(id, { name, description }, { new: true });
        if (!updated)
            return res.status(404).json({ message: 'Playlist não encontrada.' });
        res.json(updated);
    }
...
router.delete('/playlists/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const deleted = await Playlist_1.PlaylistModel.findByIdAndDelete(id);
        if (!deleted) {
            return res.status(404).json({ message: 'Playlist não encontrada' });
        }
